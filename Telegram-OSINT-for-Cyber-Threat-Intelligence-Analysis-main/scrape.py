...
parser.add_argument(
    ...
    nargs="+",
    type=str,
    help="Specify entity usernames or IDs to collect messages and participants from (i.e.: --entities <id1> <username2>)",
)
parser.add_argument(
    "--debug",
    action="store_true",
...
if __name__ == "__main__":
    ...
    try:
        ...
        with TelegramClientContext() as client:
            ...
            # Entity IDs/usernames from which to scrape, if specified in CLI arguments
            entities_to_scrape: list[str] | None = (
                args.entities if args.entities else None
            )
            ...
            for dialog in client.iter_dialogs():
                ...
                # Check if this entity is in the allowed list (username)
                if entities_to_scrape and hasattr(entity, 'username') and entity.username in entities_to_scrape:
                    # Proceed with collection
                    pass
                elif entities_to_scrape and str(entity.id) in [str(e) for e in entities_to_scrape]:
                    # Proceed with collection (if ID was provided)
                    pass
                elif entities_to_scrape:
                    # If specific entities were requested and this isn't one of them, skip
                    continue
                
                if not (args.max_entities and entities_collected >= args.max_entities):
